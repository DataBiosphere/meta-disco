...
def name_tokens(name: str) -> list[str]:
    ...
    ``interval``, ``chm13v2``. One definition, shared with `slot_map`, so no caller
    tokenizes a name differently.
    """
...
def render_report(survey: Survey) -> str:
    ...
    out: list[str] = [
        ...
        "comes from `data/anvil/prod/manifest/`. Regenerate with `make manifest-survey`; the same "
