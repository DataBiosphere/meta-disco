...
linkml_meta = LinkMLMeta({'default_prefix': 'anvil',
     ...
     'description': 'The contract for a raw AnVIL file metadata record — one entry '
                    ...
                    'are what AnVIL publishes today, not input to inference, and '
                    'the published importer writes them as evidence (claims '
                    'contract 4.1 kind 2, #497) from the `anvil_file` table (today '
                    'via the verbatim manifest, 7.12), never from this record. '
                    'They were modeled here until #424 and dropped one step before '
                    'classification, which is the bug that issue describes; '
                    'modeling them as input slots is what invited it. Since #513 '
                    'the downloader no longer copies them onto a record, and an '
                    'input file written before that still carries them as '
                    'unmodeled extra keys, which the input model ignores.\n'
