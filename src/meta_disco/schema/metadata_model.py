...
linkml_meta = LinkMLMeta({'default_prefix': 'anvil',
     ...
     'description': 'The contract for a raw AnVIL file metadata record — one entry '
                    ...
                    '4.1 kind 2, #497) from the TDR `anvil_file` table, which no '
                    'run reads yet (#432), never from this record. They were '
                    'modeled here until #424 and dropped one step before '
                    'classification, which is the bug that issue describes; '
                    'modeling them as input slots is what invited it. They still '
                    'arrive on every record, as unmodeled extra keys, and the '
                    'output model is where they are modeled — '
