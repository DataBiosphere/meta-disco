...
linkml_meta = LinkMLMeta({'default_prefix': 'anvil',
     ...
     'description': 'The contract for a raw AnVIL file metadata record — one entry '
                    ...
                    'which this project diffs its own answer against — and they '
                    'enter as an input (claims contract 4.1 kind 2, #472/#497) '
                    'through the published importer, which reads them from the TDR '
                    '`anvil_file` table into evidence files, not through this '
                    'record. They were modeled here until #424 and dropped one '
                    'step before classification, which is the bug that issue '
                    'describes; modeling them as input slots is what invited it. '
                    'They still arrive on every record, as unmodeled extra keys, '
                    'and the output model is where they are modeled — '
