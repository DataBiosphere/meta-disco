...
class EvidenceRow(ConfiguredBaseModel):
    ...
    **Where this gate is weaker than the reader**, which is the same divergence `EvidenceFileEnvelope` documents and for the same cause. `column` is optional, LinkML models an optional slot as nullable (`gen-json-schema` emits `type: [string, null]`), so `{\"column\": null}` is valid here by construction — and `_entry_from_line` refuses it through `member_optional_str`, because the writer omits a member it does not have and a written-out null is a line this writer could not have produced.
