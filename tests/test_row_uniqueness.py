...
class TestRowIdentities:
    def test_a_file_id_in_two_producers_outputs_is_a_duplicate(self, tmp_path):
        write_run(tmp_path, fname="tar_classifications.json", records=[_row("f1"), _row("f2")])
        write_run(tmp_path, fname="auxiliary_classifications.json", records=[_row("f1")])
    ...
    def test_a_file_id_written_twice_by_one_producer_is_a_duplicate_too(self, tmp_path):
        write_run(tmp_path, fname="bam_classifications.json", records=[_row("f1"), _row("f1")])
    ...
    def test_distinct_file_ids_are_no_duplicates(self, tmp_path):
        write_run(tmp_path, fname="bam_classifications.json", records=[_row("f1"), _row("f2")])
        write_run(tmp_path, fname="vcf_classifications.json", records=[_row("f3")])
    ...
    def test_rows_with_no_file_id_are_counted_not_paired(self, tmp_path):
        ...
        write_run(tmp_path, fname="bam_classifications.json", records=[_row(None), _row(""), _row("f1")])
    ...
    def test_the_scan_keys_on_the_field_it_is_told(self, tmp_path):
        ...
        write_run(tmp_path, fname="bam_classifications.json", records=[hprc_row, dict(hprc_row, file_name="HG003.bam")])
...
class TestTheRunFailsOnDuplicates:
    def test_a_duplicate_fails_the_run_and_names_it(self, tmp_path, capsys):
        write_run(tmp_path, fname="tar_classifications.json", records=[_row("f1")])
        write_run(tmp_path, fname="auxiliary_classifications.json", records=[_row("f1")])
    ...
    def test_rows_without_a_file_id_do_not_fail_the_run(self, tmp_path, capsys):
        ...
        write_run(tmp_path, fname="bam_classifications.json", records=[_row(None), _row(None)])
    ...
    def test_an_hprc_run_is_checked_on_its_own_key(self, tmp_path, capsys):
        ...
        write_run(tmp_path, fname="bam_classifications.json", records=rows)
        ...
        write_run(tmp_path, fname="tar_classifications.json", records=[rows[0]])
