...
def write_input(tmp_path: Path, repository: str = "anvil", catalog: str | None = "anvil15") -> Path:
    metadata: dict = {"repository": repository}
    if catalog is not None:
        metadata["catalog"] = catalog
    path = tmp_path / "input.json"
    path.write_text(json.dumps({"metadata": metadata, "files": []}))
    return path
...
def write_evidence(
    ...
    *,
    source_type: str = SOURCE_REPOSITORY_METADATA,
    ...
    """One evidence file of ``(field, target_key_value, raw_value)`` lines, in the generation layout."""
    source = EvidenceFileSource(repository="anvil", dataset=dataset, table=table, url="https://example.org")
    kind = "anvil_published" if source_type == SOURCE_PUBLISHED_VALUE else "anvil"
    directory = generation_dir(root, kind, version, dataset, STAMP)
    ...
    path = evidence_file_path(directory, table)
    write_evidence_file(path, envelope, entries)
    return path


def published(root: Path, lines: list[tuple[str, str, str]]) -> Path:
    return write_evidence(root, lines, source_type=SOURCE_PUBLISHED_VALUE, table=PUBLISHED_TABLE, column=lines[0][0])
...
def test_ac4_the_report_lists_the_join_per_source_and_dataset(tmp_path, run, evidence, table):
    ...
    result = go(run, tmp_path, evidence, table)
    rows = {(e["source_type"], e["dataset"]): e for e in result.report["evidence"]}
    ...
    assert on_disk["evidence"] == result.report["evidence"]
...
def test_evidence_for_another_catalog_is_refused(tmp_path, run, evidence, table):
    write_run(run, [record(1)])
    write_evidence(evidence, [("platform", drs(1), "PACBIO_SMRT")], version="anvil14")
    with pytest.raises(ReconcileError, match="anvil14"):
        go(run, tmp_path, evidence, table)
...
def test_evidence_about_another_system_is_skipped_and_a_repository_without_a_catalog_is_not_checked(
    ...
    assert result.report["evidence"] == [] and len(result.report["skipped_evidence"]) == 1
...
def test_ac7_a_seeded_row_makes_no_claim(tmp_path, run, evidence, table):
    ...
    assert result.report["inputs"][DATASET]["platform"][SOURCE_REPOSITORY_METADATA] == {"unreviewed": 1}
...
def claim(value=None, status=None, source_type=SOURCE_REPOSITORY_METADATA, raw=None) -> dict:
    out = {"source_type": source_type, "raw_value": raw if raw is not None else (value or status)}
...
def test_ac8_agreement_classifies():
    assert resolve_slot(CLASSIFIED, "GRCh38", [claim("GRCh38")], False) == (CLASSIFIED, "GRCh38")


def test_ac9_a_source_fills_a_gap_inference_left():
    assert resolve_slot(NOT_CLASSIFIED, None, [claim("PACBIO")], False) == (CLASSIFIED, "PACBIO")
...
def test_ac11_not_applicable_beside_a_value_is_a_conflict():
    assert resolve_slot(NOT_APPLICABLE, None, [claim("GRCh38")], False) == (CONFLICT, None)
    assert use_for(CONFLICT) == USE_PUBLISHED


def test_ac12_not_classified_yields_to_not_applicable():
    assert resolve_slot(NOT_CLASSIFIED, None, [claim(status=NOT_APPLICABLE)], False) == (NOT_APPLICABLE, None)
...
def test_ac13_two_agreeing_sources_and_a_silent_inference():
    ...
    assert resolve_slot(NOT_CLASSIFIED, None, two, False) == (CLASSIFIED, "PACBIO")


def test_ac14_an_inference_conflict_stands_beside_a_source_value():
    assert resolve_slot(CONFLICT, None, [claim("PACBIO")], False) == (CONFLICT, None)
...
def test_ac15_without_evidence_reconcile_concludes_what_inference_did_and_leaves_it_untouched(tmp_path, run, table):
    ...
    assert result.report["evidence_excluded"] is True
...
def test_ac23_use_follows_the_status_and_value_is_null_unless_classified():
    ...
    for status, value in (
        resolve_slot(CLASSIFIED, "GRCh38", [claim("CHM13")], False),
        resolve_slot(NOT_CLASSIFIED, None, [], True),
        resolve_slot(NOT_APPLICABLE, None, [], False),
    ):
...
def test_ac24_a_harmonized_published_value_agrees_and_is_scored_harmonized(tmp_path, run, evidence, table):
    ...
    scored = result.report["inputs"][DATASET]["reference_assembly"]
...
def test_ac25_an_unreviewed_published_value_beside_another_input_is_a_conflict(tmp_path, run, evidence, table):
    ...
    ref = slot(run, 1, "reference_assembly")
    assert (ref["status"], ref["value"], ref["use"]) == (CONFLICT, None, USE_PUBLISHED)
    assert result.report["inputs"][DATASET]["reference_assembly"][SOURCE_PUBLISHED_VALUE] == {"unreviewed": 1}
...
def test_ac26_an_unreviewed_published_value_alone_is_missing_work_not_a_conflict(tmp_path, run, evidence, table):
    ...
    counts = result.report["slots"][DATASET]["reference_assembly"]
...
def test_the_report_scores_inference_and_counts_filled_and_agreed_slots(tmp_path, run, evidence, table):
    ...
    result = go(run, tmp_path, evidence, table)
    counts = result.report["slots"][DATASET]["platform"]
    assert (counts["agreed"], counts["filled"], counts["inference_only"]) == (1, 1, 1)
    assert result.report["inputs"][DATASET]["platform"]["inference"] == {"agreed": 1, "added": 1, "silent": 1}
    assert result.report["conflict_rate"][DATASET]["platform"] == 0.0
...
def test_a_source_is_not_scored_on_a_dataset_its_evidence_does_not_cover(tmp_path, run, evidence, table):
    ...
    write_evidence(evidence, [("platform", drs(1), "PACBIO_SMRT")])
    result = go(run, tmp_path, evidence, table)
    assert SOURCE_REPOSITORY_METADATA not in result.report["inputs"]["AnVIL_OTHER"]["platform"]
