...
Candidate names are matched **case-insensitively** (#455), as routing matches extensions (#449). So an index file and its parent may disagree about case — `SAMPLE.BAI` finds `SAMPLE.BAM` — and, conversely, two files in one dataset whose names differ only by case identify neither, which is the ambiguous case above. `parent_names_matched` is what tells the two apart: one name is a true duplicate, several mean case was the difference. Folding changed no row of the anvil15 snapshot measured for #455: re-running the producer over it produced all 224,260 records unchanged and the same matched / unmatched / ambiguous counts. Which files are ambiguous did not change either — but every ambiguous entry is one key wider than before, since `parent_names_matched` is new here, so the 15,058 `unmatched_files` entries compare equal only once that key is set aside. A later catalog may differ; nothing in the repo re-checks it.
