#!/usr/bin/env python3
"""Fetch FASTQ headers and classify using read name inspection.

Headers are cached in data/{repo}/evidence/fastq/ for:
...
def _evidence_dir() -> Path:
    return _repo.evidence_dir("fastq")
...
def save_header_evidence(key: str, file_name: str, read_names: list[str], raw_bytes: int):
...
def get_fastq_reads(key: str, url: str, file_name: str = "", is_gzipped: bool = True,
                    num_reads: int = 10, use_cache: bool = True) -> list[str] | None:
    """Read first N read names from a FASTQ file at a URL.
    ...
    try:
        ...
        if read_names:
            save_header_evidence(key, file_name, read_names, raw_bytes)

        return read_names if read_names else None

    except requests.Timeout:
        print(f"Timeout reading FASTQ for {file_name}")
        return None
    except Exception as e:
        print(f"Error reading FASTQ for {file_name}: {e}")
        return None
...
def classify_single_fastq(
    ...
    """Fetch reads and classify a single FASTQ file."""
    read_names = get_fastq_reads(key, url, file_name, is_gzipped, use_cache=use_cache)
    if not read_names:
        return None
...
def process_single_record(record: dict, resume: bool) -> tuple[dict | None, bool]:
    ...
    was_cached = load_cached_header(key) is not None

    result = classify_single_fastq(key, url, file_name, file_size=file_size,
...
def process_fastq_files(input_path: Path, output_path: Path, limit: int | None = None,
    ...
    # Filter to FASTQ files with a valid key
    fastq_extensions = [".fastq", ".fastq.gz", ".fq", ".fq.gz"]
    needs_inspection = [
        r for r in results
        if _repo.get_key(r)
        and any(_repo.get_file_format(r).endswith(ext) or _repo.get_filename(r).endswith(ext)
    ...
    print(f"Found {len(needs_inspection)} FASTQ files for header inspection")
...
def main():
    ...
    if args.md5:
        ...
        result = classify_single_fastq(args.md5, url, is_gzipped=is_gzipped)
        if result:
