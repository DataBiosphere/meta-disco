#!/usr/bin/env python3
"""Classify FASTQ files by read name inspection.

DEPRECATED: Use scripts/classify_headers.py --type fastq instead.
...
def classify_single_fastq(
    ...
    pipeline = ClassifyPipeline(FASTQ_CONFIG, Path("unused"), Path("unused"))
...
def main():
    ...
    if args.md5:
        pipeline = ClassifyPipeline(FASTQ_CONFIG, Path("unused"), Path(args.output))
